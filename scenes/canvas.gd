...
func _on_memory_timer_timeout() -> void:
	...
	memory_time_min_current = clamp(memory_time_min - (0.2 * memories.size()), 0, memory_time_min)
	memory_time_max_current = clamp(memory_time_max - (0.2 * memories.size()), 0.2, memory_time_max)
	print(memory_time_min_current, ", ", memory_time_max_current)
